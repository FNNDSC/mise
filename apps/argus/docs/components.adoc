...
| `actions` | The row verbs. Without a `rowZone`, declaring this mints the action track: a track of `width` on every row, empty until the row is indicated, then holding `of(row)`; with one, no track, and `of(row)` is drawn in the zone. A row `of` offers nothing keeps its single click. `always: true` draws every row's verbs at once and the click activates instead of indicating.
...
| framed | `.listing-framed` on the mount | A declared `rowZone`. The indicated row also wears a wash of the frame's hue, and the zone wears `.listing-zone`.
...
`a-row-s-verbs-live-in-the-frame`. A listing that declares a `rowZone` reserves no track: the indicated row's verbs are drawn in the frame's row zone, the row carries only its light and the frame's wash, verbs arriving open the frame, the last of them leaving retracts it unless the operator holds it, and the frame retracting stands the row down. A selection's verbs ride the same zone.
...
A theme may set, per pane: `--roster-gap` and `--roster-pad` on the grid host, the hues of `.listing-indicated` and `.listing-selected` and the framed row's wash (`.listing-framed .listing-indicated`), and the per-trait cell rules the trait's `className` names (alignment lives there, right-aligned numbers included). The row zone's capsules are `.listing-action` wearing the frame's geometry (`.mode-frame .listing-zone .listing-action`: full width, square, right-aligned, a label that wraps rather than cuts). A theme may not write `--roster-cols` on a converted pane, since the façade owns it; nor size `.listing-action`, whose height is declared once and read by the track that reserves room for it.
