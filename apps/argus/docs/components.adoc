...
| bar | `.pane-handle` | `.pane-title` `IMAGE <modality> <description>`, `.pane-mode` the layout when not single, `.pane-state` the readout: `SLICE n OF m`, `LOADING`, `MPR · m SLICES`, `REFUSED n OF m`, `UNSUPPORTED TRANSFER SYNTAX <uid>`.
...
| tool blocks | `.image-tool[data-tool]` | One per library tool: `wl`, `zoom`, `pan`, `length`, `angle`, `probe`. The bound one is lit, the rest `rail-off`.
| layout block | `.image-layout` | Cycles SINGLE, MPR, 3D.
...
`an-instruments-field-is-foreign` (no control inside `.image-field`; the lint reads the template), `focus-stays-in-the-field` (smoke), `frame-retracts-to-a-strip`, `a-control-lives-where-it-acts` (IMAGE on the row that holds the series), `hue-is-a-mode` (the modality on the frame), `a-readout-that-acts-says-so` (LOADING, REFUSED, UNSUPPORTED on the bar).
