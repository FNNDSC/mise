...
| control | `.listing-control` on the declared `control` trait's cell | A capsule that reads what it will do next — OPEN / CLOSE on a folding row, OPEN on a row that enters or shows, UP on the browser's updir; activates the row on a press and does nothing else.
