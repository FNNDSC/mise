...
| cluster | (scene) | The feeds of one shape, gathered by an unseen anchor with no mass of its own; not drawn.
| tip | `.dag-node-tip` | Over a sphere: `<plugin> ×N · <status> · feed <id> · <title>`, the status read as `<n> errored` on a fan with failures. Inside a feed, a node's own label.
...
panel.control('enter', ['4299']);         // the console's `universe enter 4299`; also `back` and `open`
----
