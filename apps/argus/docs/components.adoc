...
| sphere | (scene) | One plugin group inside one feed: size the jobs it stands for, colour the worst status; `×N` on a fan.
...
| tip | `.dag-node-tip` | Over a sphere: `<plugin> ×N · <status> · feed <id> · <title>`.
