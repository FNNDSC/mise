...
The index pill is `.listing-index` inside the `.listing-index-cell` track: a capsule of the listing's own idiom. Lit, it carries the number the session reaches, in `--listing-index-hue` — a token each pane sets, so a pill belongs to the listing it stands in rather than to one colour the whole surface shares. Unlit, it carries the row's own count on a faint ground. A theme sets the hue per pane and may not resize the track, which the façade owns.
