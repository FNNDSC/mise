...
const listing = new Listing<FsListingEntry>({
  ...
  address: (entry) => entry.path,     // what the KERNEL calls it: the pill LIGHTS for a row @N reaches
...
A pane's header is one line: its title ellipsises and carries the whole name as a tooltip, while the mode and state readouts beside it keep their width, whatever the pane is called.

The DASHBOARD (`features/launcher/panel.ts`, pane kind `launcher`, preset of its own, reached by DASHBOARD-01, the gutter's first block, or the word `dashboard`, and re-read on entry and on a fifteen-second beat while it is on stage) is not a listing: it is one block per domain, each a control. A block carries its name, figures (a count, a state, or a filter that opens the roster filtered), rows that are doors into recent work, and the verb it performs; the first block in the list takes the wide seat, so the domain with the most to say says it. Colour reaches a block as `--tile-hue` and its type is the frame's black-on-hue, so the contrast gate holds it like any other surface; nothing is faded, since a translucent chevron is a failed pairing waiting to happen. It is the empty state — `launcher_yield` hands the stage back to home the moment content opens — and `START HERE` on the block grid itself remembers whether a session begins there.
...
The roster re-asks the session on its own beat while it is on stage (ten seconds while a listed feed is live, a minute when all are terminal), so a run started anywhere settles in place; whether it is on stage is the pane's own state, never read back from a style property.
...
| numbered | `.numbered` on the row's `.listing-index` pill | The session's last answer holding this row's `address`. EVERY level leads with an index pill: a row the session reaches wears its HANDLE — the kind it counts and its place in that kind's sequence, `SER003`, `STD001`, `FIL012`, `DIR004`, padded to three or to the sequence's width — lit in the pane's frame hue; every other row wears its own place in the listing, dim, a counter and plainly not a promise since a bare number is nothing the operator can type. The handle is found BY ADDRESS, so a sorted or filtered listing keeps each row's own code rather than renumbering down the screen. A lead row (`..`) keeps the cell and shows nothing. Painted by `listingNumbering_set` and after every render.
|===
...
The index pill is `.listing-index` inside the `.listing-index-cell` track: a capsule of the listing's own idiom. Lit, it carries the number the session reaches, in `--listing-index-hue` — a token each pane sets to its own FRAME hue, because the pill is chrome rather than content: a pill in the row's own colour sits beside the row's glyph and reads as one blob. Unlit, it carries the row's own count on a faint ground. A theme sets the hue per pane and may not resize the track, which the façade owns.
...
Source: `apps/argus/src/features/universe/panel.ts` (the pane), over `features/dag/universe.ts` (the graph built from landed feeds, the tip, the memory) and `scene/chrisSpace.ts` (the orrery reading ChRIS — `@fnndsc/orrery`'s scene, shared with the DAG pane).

Status: on stage since 2026-09-22 (slice 1 of the universe descent). Opens from the dashboard's UNIVERSE tile as a stage of its own (the `universe` preset, one primary pane raised whole, like RUNS and PACS); the wait's theater in the DAG pane (a universe drawn while the roster is refused) shares its graph and its tip but is not this pane.

=== Purpose

The universe pane shows the space of everything run on this CUBE: every feed the session's index holds, each drawn as its own small molecule — one sphere per plugin group, sized by the jobs it stands for and hued by the share of them that failed — pulled toward the feeds that share its pipeline shape, so the field reads as what this lab runs, how much of each, and where the red is. That is the FEEDS view, the default: the whole compute structure. The SHAPES view folds it: one molecule per pipeline shape, its spheres the shape's stages, each sized by the feeds folded under it and hued by how many of those feeds erred there — a thousand feeds of one bespoke pipeline as one molecule, `×1,000`; a click unfolds a shape into its members in place. In either view a click on a feed's sphere descends into the feed. It is its own pane kind: RUNS-02 stays a feed viewer, and the universe keeps its camera and its memory. It breathes with the index (landings while it warms, arrivals and status changes once it is whole) and remembers where its molecules settled, per identity.

=== Anatomy

.The universe pane, numbered: the bar (1), the rule (2), the elbow (3), the spine widened into the frame (4) with its blocks (5), the field of molecules in clusters (6), and the tip over a sphere (7)
image::images/universe-pane-anatomy.svg[Universe pane anatomy,1180]

[cols="1,1,3"]
|===
| Part | Class | Holds

| pane | `.pane-universe` | The primary pane `universe`, the whole of its stage; the tile enters the stage and asks for the space once.
| bar | `.pane-handle` | `.universe-title` `UNIVERSE — n FEEDS · m SHAPES`, and while the index warms `· INDEX WARMING loaded/total (p%)`; `.pane-state` `LANDING` while the space still grows, `WHOLE` once the index is.
| frame | `.field-rule` `.mode-strip` `.mode-elbow` `.mode-frame` | The mode frame every pane wears; a strip at rest.
| projection block | `.universe-projection` | Cycles 3D and 2D.
| view block | `.universe-view` | Cycles FEEDS (every feed its own molecule, the default) and SHAPES (the shapes folded, one molecule each).
| scale block | `.universe-scale` | Cycles JOBS (a sphere's radius by the log of the jobs it stands for; in SHAPES, of the jobs across the folded feeds) and ALIKE (every sphere alike; the field reads by shape alone).
| density block | `.universe-density` | SHAPE (a stage as one sphere with its count) or CENSUS (every job of every stage as its own point, shelled around the stage in one instanced mesh; hover and a click still name the stage beneath the pointer). `universe density shape\|census` by word.
| draw block | `.universe-draw` | STARS (the default: every node a point of light — error-free stars glow additively, errored ones are embers drawn over them in plain blending so red stays red; edges faint threads in one batch; a cluster's halo a nebula glow that is still its handle) or SPHERES (every node lit and solid, as before). A feed near enough to read turns solid as a whole — its largest sphere past about seven pixels on screen, back to stars below five — crossfading over 300 ms, its threads rising to edges; its spheres exist only while it is solid. A feed you enter is always solid. `universe draw stars\|spheres` by word. The frame's four choices — draw, view, scale, density — are kept per identity (`argus.universe.settings.<user>@<uri>`); the physics knobs are play and are not kept.
| layout block | `.universe-arrangement` | GALAXY (the default: the space finds its own shape — every sphere repelling every other, springs along each feed's edges and from each feed's root to its shape's hub, no overlap, let go to rest; `scene/galaxy.ts`, starting from the spokes arrangement so the same forces refine rather than unfold), SPOKES (every feed a spoke from its hub, its chain pointing out, the reach growing with the crowd) or CLUMPS (the feeds packed round the hub). All three settle in a worker (`scene/layoutWorker.ts`; SPOKES and CLUMPS as a hierarchy, `scene/hierarchy.ts`). Each arrangement keeps its own positions, in the session and per identity (a galaxy under the universe's positions key, the others beside it as `.spokes` / `.clumps`), so a return to one already seen redraws at once. `universe layout galaxy\|spokes\|clumps` by word; kept with the other frame choices. Up close a solid feed's edges are lit tubes (red into a failed stage) carrying pulses from parent to child: a tube into a running or waiting stage streams; a finished feed replays its run in a cycle — each stage dark at the start, flashing with a glow that blooms and settles as its pulse lands, then held lit, the lot dimmed again before the run replays; a running stage blinks with each pulse that lands until it is done. A pulled sphere drags its tubes and its chain under the layout's own forces. The DAG pane draws its edges as the same tubes. A census drawn as spheres joins its jobs with them too (up to 20,000 edges; lines past that). By touch: one finger orbits (or pulls the node it lands on), a pinch dollies as the wheel does, two fingers pan, a tap selects and names, a double tap enters. CONSTELLATIONS (fourth in the cycle; `universe layout constellations`; FEEDS only — SHAPES gives way to GALAXY): every plugin a ringed star placed by what runs with what, every feed's stages pulled to the stars they ran (the rarest plugin pulls hardest, a plugin in nine feeds in ten not at all), no shape's hub; a star's tip reads `pl-x · n feeds · e% errored`; a press lights the stages it ran across the sky (every other stage faint, the stars all still pressable; pressed again, or empty space, puts it out; `universe plugin <name> \| off`), its facts read PLUGIN, FEEDS and ERRORED with the verb OPEN IN /BIN (its newest entry, opened as every /bin entry opens), a double press frames its stages; a feed entered keeps the sky behind it. DATA (fifth in the cycle; `universe layout data`; FEEDS only — SHAPES gives way to GALAXY): every feed hung from what it began from — a nebula per format, under DICOM one per modality, under that one per series-description group (descriptions grouped by their words; `dataSpace.ts`) — settled by the orrery's `hubs` engine; formats and modalities are captioned `NIfTI · 435`, a description group when it holds five feeds or two in every hundred, and where two captions meet on screen the bigger hub's is read; a hub's tip reads its path and feed count, a press lights its feeds; a feed the index has not read yet hangs from NOT YET READ.
| gravity block | `.universe-gravity` | GRAVITY ON / OFF: the mass-weighted centring that gathers the clusters. A knob for play, with the rest by word: `universe physics charge\|link\|collide\|gravity on\|off`, `universe physics reset`; refused while inside a feed or a cluster.
| refresh block | `.universe-refresh` | Asks the session for the space again (`proc universe`).
| replay block | `.universe-replay` | The space's history: every feed hidden until the day it was made, then shown with a flash where it stands now, the camera untouched; thirty seconds end to end at ×1. Names its state — REPLAY at rest, PLAYING (a press pauses), PAUSED (a press plays on), REPLAYED at the end (a press plays again); the bar's readout says the day reached (`REPLAY 2025-07-31`). The whole space only, FEEDS and SHAPE and STARS — refused by name otherwise; entering a feed, a cluster or any mode change ends it, and so does Esc. By word: `universe replay [speed] \| pause \| stop \| at <YYYY-MM-DD>`; `universe state` says where it stands.
| open block | `.universe-open` | While a feed is entered: the feed in a RUNS pane of its own (`feed diagram`).
| back block | `.universe-back` | While a feed is entered: climb out to the whole space (Esc does the same).
| facts | `.universe-facts` (a `.dag-facts`) | Inside a feed, the indicated node's payload — PLUGIN, INSTANCE, STATUS, COUNT on a fan, DATA — and its verbs, ENTER NODE (flies into the node: the camera dollies into the sphere and the node's data opens inside it as a rooted browser, the DAG pane's overlay; Esc flies out; a double click on the node does the same) and PROCESS, as `.universe-node-verbs`; hidden until a node is indicated. `universe node <instance>` flies in by word.
| field | `.universe-canvas` | The scene's mount; holds the canvas and the tip, never a control.
| sphere | (scene) | One plugin group inside one feed: size the log of the jobs it stands for (JOBS) or alike (ALIKE); colour done shading to error by the share of its jobs that failed, red outright when all did; `×N` on a fan.
| folded stage | (scene) | In SHAPES: one stage of a shape across every feed that ran it; size the log of the feeds folded (ALIKE) or of the jobs across them (JOBS); colour by the share of those feeds that erred at the stage; `×N` the feeds folded.
| molecule | (scene) | One feed: its spheres joined by edges.
| cluster | (scene) | The feeds of one shape. In FEEDS its members are gathered by an anchor with no mass of its own, drawn as a HALO (a translucent sphere sized by the feeds it gathers; hover names the shape and its count; a solid sphere under the pointer always wins the pick; a click lights the cluster). In SHAPES it IS the folded molecule, and a click unfolds it.
| tip | `.dag-node-tip` | Over a folded stage: `<plugin> · <n> feeds · <m> with errors · <shape>`. Over a member sphere: `<plugin> ×N · <status> · feed <id> · <title>`, the status read as `<n> errored` on a fan with failures. Over a halo: `<plugin> > <plugin> · <n> feeds`. Inside a feed, a node's own label.
| empty | `.universe-empty` | Before the first space arrives.
|===
...
[source,ts]
----
const panel = new UniversePanel(
  { canvas, title, state, empty, projectionPill, refreshPill },   // the template's parts
  { command_run: (line) => client.line_execute(line, { silent: true }).then(o => o.envelopes.forEach(panel.envelope_observe)) },
  localKeyStore(),                                                 // where the molecules' places are remembered
);
panel.request();                          // asks `proc universe`; the answer arrives through envelope_observe
panel.promptContext_observe(context);     // every pushed prompt: landings, the warming figure, arrivals, identity
panel.control('enter', ['4299']);         // the console's `universe enter 4299`; also `cluster <feed>`, `back` and `open`
----

The cluster: in FEEDS a click on a halo (or `universe cluster <feed>`) lights the shape's feeds where they stand, dims the rest without moving anything, and flies the camera to frame them. In SHAPES the same click unfolds the folded molecule in place into its member feeds — each starting at the fold's centre and settling out, the rest frozen and dim, the settle a feed's descent uses. BACK, Esc or `universe back` climbs one level; a feed entered from a cluster climbs back to the cluster. `universe view feeds|shapes` switches the top; it refuses while inside.

The descent: a click on a sphere (or `universe enter <feed>`) flies the camera to that molecule, asks the host for the feed's graph (`feed_dag`, the kernel's `feed diagram`, which already collapses a fan into one node with a tally — the budget), unfolds the molecule into that graph in place with every other feed dimmed, and frames the nearest four fifths of the feed (a node must be wide enough to hover; the wheel reaches the outliers). While the ask runs the bar reads `UNIVERSE — ENTERING FEED n · title …` and the pill ASKING; an ask that yields nothing says so on the console, with the feed and the seconds waited. Inside, a click on a node shows its facts and verbs; the dimmed field behind the feed takes no pointer. Esc, BACK or `universe back` climbs out to the space as it has landed since; OPEN FEED puts the feed in a RUNS pane (`feed_open`). Landings and re-asks are taken while inside — and from the click until the feed is entered — and drawn on the climb; a repaint of the same space keeps a camera the operator has wheeled or dragged (law a-click-does-not-resettle-the-space). A wait on the field wears the wait readout (`WaitProgress`, `features/wait/progress.ts`, `.wait-progress`): bottom-left over the field, taking no pointer; `ASKING THE SESSION FOR THE SPACE` with a running bar until `proc universe` answers, then `SETTLING n SPHERES · p%` with a moving bar while a large settle runs in slices (law a-wait-over-two-seconds-shows-progress). A paint holds what already stands and settles only newcomers (`SettleMode` `new`); a scale or a view re-settles everything (`full`). The space turns about its focus — the point the camera looks at, set by a fit and by every flight: the idle spin and the orbit pivot on it, the wheel dollies toward it, a pan carries it, so an entered feed or a lit cluster turns in place in the middle of the frame. A pull on a sphere moves only its own molecule.

The pane asks one kernel command, `proc universe`, and reads one prompt field, `procWarmup.landed`; it never walks a feed itself. While the index warms each landing adds or reshapes a molecule (repaint at most once a second); once whole, a feed arriving through the roster delta or the feed count moving re-asks for the space (at most once in five seconds), so a new feed drifts in and a gone one leaves. Positions are written to the store under `argus.universe.<user>@<uri>` a few seconds after each settle and seed the next settle, so a reopened pane finds its molecules where they were.

=== States

* `.pane-state` `LANDING`: the index is still warming; the space grows with each landing.
* `.pane-state` `WHOLE`: the index is whole; the space stands, and moves only when the lab does.
* `.universe-canvas` hidden and `.universe-empty` shown: no space has arrived yet.
* `.universe-inside` on the pane, `.pane-state` `INSIDE`: a feed is entered; the title reads `INSIDE FEED <id> · <name> · <n> JOBS`; the OPEN FEED and BACK blocks stand on the frame.
* `.universe-cluster` on the pane, `.pane-state` `CLUSTER`: a shape is in view; the title reads `SHAPE <plugin> > <plugin> · <n> FEEDS`; BACK stands on the frame.

=== Laws

`a-control-lives-where-it-acts` (the pane's modes on its frame; a sphere's verbs will ride its overlay), `surface-never-shadows-the-session` (`proc universe` is the kernel's), `index-movement-never-holds-the-lane` (cache-resident asks only), `field-is-content`, `frame-retracts-to-a-strip`, `drawer-everywhere`.

=== Stylesheet

A theme may set the field's background and the tip's face. It may not put a control inside `.universe-canvas`, nor size the canvas other than 100% of its mount.

=== Do not

* Do not open the universe in the DAG pane, nor split it beside a browser: RUNS-02 is a feed viewer, and the universe is a stage, entered whole.
* Do not walk feeds from the surface to draw a molecule: the shape comes from the index, landed or whole.
* Do not repaint on every prompt: landings are batched to a second, re-asks to five.
* Do not unfold a feed past the kernel's own collapse: a fan stays one `×N` sphere inside; the facts carry the true count.
* Do not invent a verb inside a feed: a node offers what the DAG pane's node offers.
...
| overlay note | `.image-overlay-note` | The header on the image while OVERLAY is on: `.image-overlay-slice` first, `SLICE n OF m` following the wheel or a two-finger swipe (hidden for a single slice), then the study's and patient's lines read once per series.
| layout block | `.image-layout` | Cycles SINGLE, MPR, 3D, SLAB.
| SLAB scene | `.image-render` `.image-viewport-render` `canvas` | SLAB's own vtk render window (not a Cornerstone viewport): a ghost volume render and a crisp `vtkImageSlice` at the current slice. Left-drag rotates, the wheel (or two fingers up or down) sweeps, the bar reads `SLAB · z n / m`; `image ghost <0..1>|off` tunes the ghost.
...
| retry block | `.image-retry` | Shown only while slices are refused, with their count (`RETRY n`); pressing it asks again for just those slices, saying ASKING… while it does, and the slice on screen is redrawn if it was among them. Slices are fetched eight at a time and each is asked again three times (after 0.5, 1.5 and 4 s) before it counts as refused; a slice with no answer in 30 s counts as refused. A slice in a transfer syntax the loader cannot read is never asked again.
