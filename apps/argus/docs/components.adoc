...
Status: on stage since 2026-09-22 (slice 1 of the universe descent). Opens from the dashboard's UNIVERSE tile; the wait's theater in the DAG pane (a universe drawn while the roster is refused) shares its graph and its tip but is not this pane.
...
| pane | `.pane-universe` | The pane; one per stage as a rule (the tile focuses a shown one rather than opening another).
...
* Do not open the universe in the DAG pane: RUNS-02 is a feed viewer, and the tile focuses a shown universe rather than opening another.
