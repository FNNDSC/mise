...
The descent: a click on a sphere (or `universe enter <feed>`) flies the camera to that molecule, asks the host for the feed's graph (`feed_dag`, the kernel's `feed diagram`, which already collapses a fan into one node with a tally — the budget), unfolds the molecule into that graph in place with every other feed dimmed, and frames the feed. Inside, a click on a node shows its facts and verbs; Esc, BACK or `universe back` climbs out to the space as it has landed since; OPEN FEED puts the feed in a RUNS pane (`feed_open`). Landings and re-asks are taken while inside and drawn on the climb.
