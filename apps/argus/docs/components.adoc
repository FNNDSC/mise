...
The universe pane shows the space of everything run on this CUBE: every feed the session's index holds, each drawn as its own small molecule — one sphere per plugin group, sized by the jobs it stands for and hued by the worst status among them — and pulled toward the feeds that share its pipeline shape, so the field reads as what this lab runs, how much of each, and where the red is. It is its own pane kind: RUNS-02 stays a feed viewer, and the universe keeps its camera and its memory. It breathes with the index (landings while it warms, arrivals and status changes once it is whole) and remembers where its molecules settled, per identity.
...
| scale block | `.universe-scale` | Cycles JOBS (a sphere's radius by the log of the jobs it stands for) and FEEDS (every sphere alike; the field reads by shape alone).
...
| sphere | (scene) | One plugin group inside one feed: size the log of the jobs it stands for (JOBS) or alike (FEEDS); colour done shading to error by the share of its jobs that failed, red outright when all did; `×N` on a fan.
| molecule | (scene) | One feed: its spheres joined by edges.
| cluster | (scene) | The feeds of one shape, gathered by an anchor with no mass of its own, drawn as a HALO: a translucent sphere at the anchor's place sized by the feeds it gathers, the cluster's handle — hover names the shape and its count, a click brings the cluster into view. A solid sphere under the pointer always wins the pick over the halo around it.
| tip | `.dag-node-tip` | Over a sphere: `<plugin> ×N · <status> · feed <id> · <title>`, the status read as `<n> errored` on a fan with failures. Over a halo: `<plugin> > <plugin> · <n> feeds`. Inside a feed, a node's own label.
...
The cluster view: a click on a halo (or `universe cluster <feed>`, the cluster of that feed) lights every feed of the shape where it stands, dims the rest without moving anything, and flies the camera to frame the cluster; BACK, Esc or `universe back` returns to the whole. A descent from a cluster climbs back to the cluster, not past it.
