...
const listing = new Listing<FsListingEntry>({
  ...
  address: (entry) => entry.path,     // what the KERNEL calls it: the row gains an index pill
...
| numbered | `.numbered` on the row's `.listing-index` pill | The session's last answer holding this row's `address`. A level that declares `address` leads with an index pill so a row can be named by the number beside it (`gather add @2,3`); the pill is found BY ADDRESS, so a sorted or filtered listing keeps each row's own number rather than renumbering down the screen. A row the numbers do not reach keeps the column and shows nothing, so the geometry never moves — the bargain the action track already makes. Painted by `listingNumbering_set`, which the surface calls with the kernel's `numbered` message.
|===
...
The index pill is `.listing-index` inside the `.listing-index-cell` track: a capsule of the listing's own idiom, carrying the number when the row is numbered and nothing when it is not (its hue is a token the theme sets; the empty state is a faint ground, never a border that moves). A theme may recolour it and may not resize the track, which the façade owns.
