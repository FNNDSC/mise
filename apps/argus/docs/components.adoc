...
| fold control | `.listing-fold` on the declared `fold` trait's cell | A capsule that reads what it will do next (OPEN, CLOSE), drawn by the stylesheet from the group's open state; folds the row on a press and does nothing else.
...
| row zone | `.listing-zone` (the pane's `<prefix>-row-zone`, declared as `rowZone`) | On the mode frame beneath the blocks that act on the field: the indicated row's verbs one under another with its readout beneath, or the selection's verbs while SELECT is on. Empty and hidden when nothing is indicated or selected; verbs arriving open the frame.
|===
...
| `rowZone` | The frame's row zone. Declared, the listing's verbs ride the frame: no level mints a track (`actions.width` is not read), the mount wears `.listing-framed`, the indicated row's verbs are drawn in the zone and its readout beneath them, and a selection's verbs ride the same zone. Verbs arriving open the frame the zone stands on (`data-modes` on its `.workspace-pane`); the last of them leaving retracts it unless the operator opened it or has since pressed another block on it; the frame retracting stands the row down. Not found by prefix: declaring it changes the grid, so it is declared.
...
| `actions` | The row verbs. Without a `rowZone`, declaring this mints the action track: a track of `width` on every row, empty until the row is indicated, then holding `of(row)`; with one, no track, and `of(row)` is drawn in the zone. A row `of` offers nothing keeps its single click — a lead row too, so `..` enters on one click while `.` is offered the place's verbs and indicates. A row with a `child` and a declared `fold` keeps navigating apart from selecting: its fold cell folds and selects nothing, the rest of the row selects and folds nothing; without `fold` its click folds and indicates at once. No double-click on a folding row. `always: true` draws every row's verbs at once and the click activates instead of indicating.
...
| `fold` | On a level with a `child`: the key of the trait whose cell is the fold control. The cell wears `.listing-fold`, folds the row on a press and does nothing else; a press elsewhere on the row indicates it without folding. Draw the cell as a capsule that reads what it will do next (OPEN / CLOSE, from the group's open state), ≥24px, inset so a miss lands on nothing; the façade refuses a `fold` naming no trait.
...
A block is `{ key, header?, lead?, rows }`. A flat listing passes one; a browser listing several directories passes one per directory with a header each. `lead` rows are drawn before the ordered rows and stand outside the order: never sorted, filtered, counted or selected. Whether one has verbs is the roster's call as for any row: the browser's `..` has none and one click activates it; its `.`, the place's own row, is offered NEW DIR / REFRESH / DELETE and indicates. Both wear their label on a capsule (UP, HERE) and no name beside it; UP, being navigation and never the block, stays lit while the field recedes. A painter (`painter_set`) draws a block some other way, cards say, and receives the rows already ordered; a painted block has no grid, so its rows carry no track and no indication, and the painter owns what a click means.

As the browser uses it: one block per listed directory, each led by its `..` as a lead row; the row type carries the entry and the full path it sits at, since a listed name does not know its own directory. The card and preview projections are a painter; the cwd binding, the stale readout and the content views (a file, an image, a /bin entry) are the browser's own, layered around the listing rather than inside it. The browser declares its row zone, so its rows carry no track: a file's click puts DOWNLOAD / MOVE / COPY / DELETE in the frame and opens it, a directory is offered no verbs and one click enters it, and a DICOM series folder keeps IMAGE and the split. A node overlay declares no verbs and no zone, so it keeps its single click.

As PACS uses it: three levels, patient over study over series, one row zone for all three; the fold glyph is each folding row's own uncapped leading trait, drawn by the stylesheet from the group's open state; a level of one row is opened with `open_set`; a patient's and a study's fold cell (`fold: 'fold'`) folds the row and nothing else, a study's body puts PULL STUDY in the frame, a series click puts GATHER / IMAGE / DIR / PULL there, and `indication_refresh` re-lights them on every stage change; the workspace carries the study template from `template_get(1)` for the form, whose QUERY stands in the SERIES column; the summed progress bars and the per-series badges remain the pane's, painted into mounts its own cells register.
...
| indicated | `.listing-indicated` on the row | A click on a row of a listing with actions (outside its fold cell, when it has one), or `row_indicate(key)`. Exactly one row at a time across every level; the previous row stands down. Belongs to the field: restored after a repaint of the same field (a fold, a sort, a re-listing), gone with a row that left, cleared by navigation. `indication_refresh()` redraws its verbs from their live predicates.
...
A theme may set, per pane: `--roster-gap` and `--roster-pad` on the grid host, the hues of `.listing-indicated` and `.listing-selected` and the framed row's block (`.listing-framed .listing-indicated`: the frame's block colour with black type), and the per-trait cell rules the trait's `className` names (alignment lives there, right-aligned numbers included). The row zone's capsules are `.listing-action` wearing the frame's geometry (`.mode-frame .listing-zone .listing-action`: full width, square, right-aligned, a label that wraps rather than cuts). A control IN a row — a fold cell, the browser's UP and HERE — is a capsule in the frame's hue, dim at rest, lit on hover, inverted on the indicated block; never a bare glyph. The browser's kind glyphs and per-kind colours are the console's (`--console-<name>`, published by the terminal from its own palette; the Nerd Font icons of `colors.yml`), held equal by a unit test. A theme may not write `--roster-cols` on a converted pane, since the façade owns it; nor size `.listing-action`, whose height is declared once and read by the track that reserves room for it.
