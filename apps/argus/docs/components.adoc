...
| numbered | `.numbered` on the row's `.listing-index` pill | The session's last answer holding this row's `address`. EVERY level leads with an index pill: a row the session reaches wears the number `@N` takes, lit in the listing's own hue; every other row wears its own place in the listing, dim — a counter, not a promise that anything answers to it. The distinction is the hue, never the digits. The lit number is found BY ADDRESS, so a sorted or filtered listing keeps each row's own number rather than renumbering down the screen, and numbers are zero-padded to the widest in their group so the column is a column. A lead row (`..`) keeps the cell and shows nothing. Painted by `listingNumbering_set` and after every render.
|===
