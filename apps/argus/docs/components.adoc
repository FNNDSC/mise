...
| bar | `.pane-handle` | `.pane-title` `IMAGE <modality> <description>`, `.pane-mode` the layout when not single, `.pane-state` the readout: `OPENING`, `SLICE n OF m`, `LOADING`, `MPR · m SLICES`, `REFUSED n OF m`, `UNSUPPORTED TRANSFER SYNTAX <uid>`, `NOT A READABLE SERIES`.
...
| load block | `.image-load` | Shown while a volume layout waits on the guard; pressing it consents and applies the layout.
