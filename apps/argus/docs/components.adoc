...
| tool blocks | `.image-tool[data-tool]` | One per tool: `wl`, `zoom`, `pan`, `length`, `angle`, `probe`. The bound one is lit, the rest `rail-off`. Five are the library's; `probe` is the pane's own readout (below).
