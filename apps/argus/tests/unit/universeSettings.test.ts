...
describe('universeSettings_parse', () => {
  it('opens a first visit as stars, every feed, sized by jobs, a sphere per stage', () => {
    expect(universeSettings_parse(null)).toEqual({ draw: 'stars', view: 'feeds', scale: 'jobs', density: 'shape' });
    expect(UNIVERSE_SETTINGS_DEFAULT.draw).toBe('stars');
  });

  it('keeps what was chosen', () => {
    const kept: string = JSON.stringify(universeSettings_of('spheres', 'shapes', 'feeds', 'census'));
    expect(universeSettings_parse(kept)).toEqual({ draw: 'spheres', view: 'shapes', scale: 'feeds', density: 'census' });
  });

  it('takes the default for anything missing or unrecognised, and survives a damaged record', () => {
    expect(universeSettings_parse(JSON.stringify({ draw: 'spheres', view: 'nebulae' }))).toEqual({ draw: 'spheres', view: 'feeds', scale: 'jobs', density: 'shape' });
