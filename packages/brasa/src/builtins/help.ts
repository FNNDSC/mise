...
export const helpText: Record<string, CommandHelp> = {
  ...
  plugin: {
    ...
    options: [
      ...
      '',
      'SUBCOMMANDS:',
      '  run [--detach] <searchable> [params]  Run a plugin and wait for it to finish',
      '                               (--detach returns the instance id at once)',
  ...
  feed: {
    ...
    examples: [
      ...
      'feed tree 1669',
      'feed diagram                       # Current feed, from inside feed_N',
      'feed note 42',
  ...
  pipeline: {
    ...
    options: [
      ...
      '  run <name|id>         Run a pipeline on the context node and wait for it',
      '  source <name|id>      Display the pipeline YAML source file',
      '  diagram <name|id>     Draw the pipeline as a shallow tree',
      '    --withargs          Include stored non-null plugin defaults inline',
      '    --signalflow        Emit SignalFlow YAML instead of the shallow tree',
      '  delete <id>           Remove a pipeline',
      '',
      'RUN OPTIONS:',
      '  --detach              Return the instance ids at once, without waiting',
    ...
    examples: [
      ...
      'pipeline diagram --signalflow PHI_detection | signalflow -',
      '',
  ...
  'pacs query': {
    usage: 'pacs query <Key:Value[,Key:Value...]> [--table | --csv | --csv-to <cfs-path>] [--series] [--title <title>] [--pacsserver <id>] [--fresh]',
    ...
    options: [
      ...
      '--force              Overwrite the table at that path; without it, an existing file is kept',
      '--table              Show the answer\'s rows as an aligned table: the CSV\'s columns, for a screen',
      '--series             One row per series instead of per study (on the table and in the CSV);',
      '                     alone, it shows the series table',
      '--anon               Stand in for names and record numbers (for demonstrating on a live PACS)',
    ...
    examples: [
      ...
      'pacs query PatientID:1234 --table      # the rows, aligned, one per study',
      'pacs query PatientID:1234 --series     # one row per series',
      'pacs query PatientID:1234 --csv-to ~/audit.csv --table --series',
      '',
  ...
  'pacs pull': {
    ...
    options: [
      '--detach      Fire retrieves and return the handles at once; prints <seriesUID> <retrieveId> per line',
      '              (--nowait is the older spelling of the same flag)',
  ...
  play: {
    ...
    options: [
      'OPTIONS:',
      '  --param NAME=value   Supply a parameter the manifest declares (repeatable)',
      '  --pace <duration>    Wait between lines, for watching a play at a surface',
      '  --dry-run            Expand the lines and show them without running any',
      '',
      'A MANIFEST:',
      '  # a comment',
      '  @name         what this workflow is',
      '  @description  one line about it',
      '  @param        MRN            a parameter it needs',
      '  @param        LABEL = draft  one with a default',
      '  <any kernel line>',
      '',
      'PRONOUNS a line may use:',
      '  ${gather}        every gathered member, as operands (where it was gathered FROM)',
      '  ${gather.place}  the same members, where they LANDED in CUBE',
      '  ${gather.first}  ${gather.last}  ${gather.3}  ${gather.size}  ${gather.name}',
    ...
    examples: [
      'play ~/flows/sag-cohort.mise --param MRN=1234567',
      'play ~/flows/sag-cohort.mise --dry-run',
      'play ~/flows/battery.mise --pace 2s',
    ],
  },
  expect: {
    usage: 'expect <gather|feed|run|path> [<target>] <property> [<predicate> <value>] [--within <duration>]',
    description: 'State a claim about the session, and refuse when it does not hold',
    options: [
      'SUBJECTS:',
      '  gather size|pulled|name          The cohort this session is working on',
      '  feed <id> status                 A feed, as this session indexed it',
      '  run <instance-id> status         One plugin instance',
      '  path <p> exists                  Whether a path is there',
      '  path <p> count [--matching <glob>]  How many things a folder holds',
    ...
    examples: [
      ...
      'expect run 601152 status eq finishedSuccessfully --within 20m',
      'expect path ~/feeds/feed_4599/pl-dcm2niix_1/data count --matching "*.nii*" gt 0',
      'expect path ~/anon tags PatientID eq anon-001',
    ],
  },
  gather: {
  ...
  query: {
    ...
    examples: [
      ...
      '',
      '# A cohort audit, as a table a spreadsheet reads',
      'query --patients @~/cohorts/ddh.txt --csv > audit.csv    # a local terminal',
      'query --patients @~/cohorts/ddh.txt --csv-to ~/audits/ddh.csv   # any surface',
      '',
  ...
  pull: {
    usage: 'pull [--detach] [--retry N] [--new-feed <title> [--plugin <selector>|--pipeline <selector>] [-- <args...>]] <vfs-path|query-expr> [...]',
    ...
    examples: [
      ...
      '',
      '# Detached: take the handles and carry on',
      'pull --detach /net/pacs/queries/42_AccessionNumber:12345678',
