...
import { listingItems_fromVfs } from './listing.js';
...
export class VFS {
  ...
  async data_get(targetPath?: string, options: { sort?: 'name' | 'size' | 'date' | 'owner', reverse?: boolean, directory?: boolean } = {}): Promise<Result<ListingItem[]>> {
    try {
      ...
      if (vfsResult.ok) {
        const items: ListingItem[] = listingItems_fromVfs(vfsResult.value);
  ...
  private async directoryEntry_get(
    ...
    if (cached) {
      ...
      if (parentResult.ok) {
        parentItems = listingItems_fromVfs(parentResult.value);
