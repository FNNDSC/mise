...
ChELL is a consumer of the **ChILI Library**. It strictly adheres to the "Sandwich Model" architecture with a hardened API boundary:
...
*   Only **cumin** talks to `@fnndsc/chrisapi`. It owns the client, shapes, and conversions.
*   **salsa** and **chili** consume cumin-owned types (e.g., `ChrisPathNode`, `ChrisPluginSummary`) and never import `@fnndsc/chrisapi` directly.
*   This keeps external API surface centralized, reduces churn from upstream changes, and clarifies responsibility boundaries.
