...
* **Syntax highlighting** - Colors popular source and configuration formats in interactive terminals
...
* `--highlight` - Force highlighting and infer the grammar from the filename or content
* `--highlight=language` - Force a specific grammar, for example `python`, `typescript`, or `sql`
* `--no-highlight` - Disable automatic highlighting and return raw text
...
When output goes directly to an interactive terminal, `cat` automatically
highlights recognized text files. Python is supported along with these common
families:

* JavaScript, JSX, TypeScript, and TSX
* Shell, SQL, Markdown, HTML/XML/SVG, and CSS/Sass/Less
* JSON, YAML, TOML, INI, and `.env` configuration
* C/C++, Java, Go, Rust, Ruby, PHP, Swift, Kotlin, R, Lua, and Perl
* Protocol Buffers, unified diffs/patches, Dockerfiles, and Makefiles

Use a bare flag to force filename inference or content auto-detection even when
standard output is not a TTY:

```bash
cat analysis.py --highlight
cat source-without-extension --highlight
```

Select a grammar explicitly when the filename is absent or misleading:

```bash
cat source --highlight=python
cat query.txt --highlight=sql
```

Use `--no-highlight` when raw terminal text is preferred:

```bash
cat config.json --no-highlight
```

Automatic mode never adds ANSI sequences to non-interactive output. ChELL's
pipe and redirect capture also strips ANSI, so downstream commands and saved
files remain plain text. Binary mode always streams the original bytes and does
not apply syntax highlighting.
...
```bash
cat /SERVICES/PACS/PACSDCM/7654321-DOE-19700101/study/series/image.dcm
```
...
* **Type detection:** Extension- and conventional-basename lookup (O(1))

=== Binary Auto-Detection Algorithm
...
=== Syntax Auto-Detection Algorithm

1. If `--no-highlight` is present: return text unchanged
2. If `--highlight=language` is present: validate and use that grammar
3. If output is non-interactive and highlighting was not forced: return text unchanged
4. Infer a grammar from the extension or conventional basename
5. For a bare forced `--highlight` with an unknown filename: auto-detect from content
6. Render recognized tokens with the shared high-contrast terminal palette

=== Data Flow

**Text mode:**
```
ChRIS API → UTF-8 decode → optional syntax highlight → command envelope → terminal
```
