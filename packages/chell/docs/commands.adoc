...
The `cat` command intelligently handles different file types based on their path:

* **Regular files** - Standard ChRIS filesystem files (e.g., `/home/user/data.txt`)
* **Pipeline files** - Pipeline specifications in `/PIPELINES/<owner>/*.yml`
* **Future types** - PACS files and other specialized content will be supported

The system automatically detects the file type and uses the appropriate API endpoint to retrieve content. Users don't need to know the underlying storage mechanism.
...
* Different file types use different retrieval methods transparently

**Architecture:**

For details on how file content retrieval works across different file types, see:

* `salsa/docs/file-content-retrieval.adoc` - Comprehensive architecture
