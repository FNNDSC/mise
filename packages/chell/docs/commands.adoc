...
**Syntax:**
```bash
pacs pull [--nowait] [--retry N] [--new-feed <title>] <vfs-path|query-expr> [...]
```
...
Planned `--plugin` and `--pipeline` flags will optionally attach one analysis to
the new root. They are not implemented or advertised in runtime help yet. See
link:pacsqr.adoc#planned-analysis-attachment[PACS Query / Retrieve / Resolve]
for the complete current behavior and planned attachment contract.
