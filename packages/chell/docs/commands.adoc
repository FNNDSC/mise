...
ChELL supports three execution modes for different use cases:

* **Interactive Mode** - Traditional REPL shell with command history and tab completion
* **Command Mode** (`-c`) - Execute single or multiple commands and exit
* **Script Mode** (`-f`) - Run .chell script files with full automation support
...
# Command mode - multiple commands with semicolons
chell -c "cd /PIPELINES; ls; cat pipeline.yml"

# Script mode - explicit
...
# Shebang execution
chmod +x script.chell && ./script.chell
```

For complete documentation on semicolon support, script files, shebang execution, error handling modes, and automation patterns, see:

**→ link:execution.adoc[ChELL Execution Modes Documentation]**
...
* **PACS Query and Retrieval** - Finding and materializing medical-imaging series
...
ChELL expands unquoted glob-style wildcard words before it dispatches a command.
The expansion is against the current CFS/VFS directory, including virtual
directories such as `/bin`, so filesystem-facing commands receive the same
concrete path arguments without maintaining their own wildcard lists. `*`, `?`,
and character classes are supported for one directory listing; an unmatched
pattern remains literal. Quote or escape wildcard syntax to pass it literally.

`upload` and `download` are explicit realm boundaries: `upload` expands its
host-local source itself, while `download` expands only its CFS/VFS source and
keeps its host-local destination literal.
...
ls /bin/pl-*             # List matching virtual plugin executables
...
Displays the contents of one or more files with support for both text and binary modes.
...
cat /PIPELINES/user/pipeline_spec.yml      # Display pipeline file

# Binary mode (DICOM medical imaging)
cat /SERVICES/PACS/.../study.dcm > local.dcm              # Save locally
cat /SERVICES/PACS/.../image.dcm | dcmdump -              # Pipe to analysis
cat --binary custom_format.dat                            # Force binary mode
```

**Binary Mode & Auto-Detection:**

Binary mode is automatically enabled for medical imaging and other binary file types:
...
* **Regular files** - Standard ChRIS filesystem files
* **Pipeline files** - Pipeline specifications in `/PIPELINES/<owner>/*.yml`
* **PACS files** - DICOM medical imaging in `/SERVICES/PACS/PACSDCM/`

**Key Features:**

* **Backpressure handling** - Ensures complete transfer of large files (tested with 150MB+)
* **Piping support** - Works seamlessly with external tools (dcmdump, convert, etc.)
* **Smart TTY detection** - Shows info messages only in interactive terminals
* **Integrity preservation** - Byte-perfect binary transfer for medical imaging
...
* Large files streamed efficiently (not loaded into memory)

**Detailed Documentation:**

For comprehensive documentation including DICOM workflows, piping patterns, and troubleshooting:

* **`chell/docs/cat.adoc`** - Complete cat command guide
* `salsa/docs/file-content-retrieval.adoc` - File retrieval architecture
...
* `local_path_or_glob` - Path or glob on the local transfer endpoint filesystem
...
* A file source is placed inside an existing target directory; it does not replace that directory
* A directory source creates a nested directory named after the source and asks before merging with an existing nested target
* In `chell --remote`, the local endpoint is currently the daemon host, not the client machine; use an absolute daemon-host path
* Status, errors, and any confirmation are rendered by the issuing surface rather than the daemon terminal

=== download

Downloads a ChRIS file or directory to the local filesystem.

**Syntax:**
```bash
download <chris_path_or_glob> <local_path> [-f|--force]
```

**Parameters:**

* `chris_path_or_glob` - ChRIS path, or a CFS glob that selects multiple paths
* `local_path` - Exact local destination. For a source glob, this is a directory receiving every match.

**Examples:**
```bash
download /home/alice/results/*.nii /tmp/results/
download /home/alice/feeds/feed_42 ./downloads/
```

== PACS Query and Retrieval

=== pacs query

Creates a PACS query, waits for its results, and prints the navigable VFS path
under `/net/pacs/queries/`.

**Syntax:**
```bash
pacs query <DICOM-key:value> [...] [--title <title>] [--pacsserver <id>] [--table]
```
...
Retrieves the series selected by one or more query, study, or series VFS paths.
A query expression is also accepted as query-then-pull shorthand.

**Syntax:**
```bash
pacs pull [--nowait] [--retry N] [--new-feed <title>
          [--plugin <selector>|--pipeline <selector>] [-- <args...>]]
          <vfs-path|query-expr> [...]
```

**Examples:**
```bash
pacs pull /net/pacs/queries/.../Study_.../Series_...
pacs pull AccessionNumber:12345678
pacs pull /net/pacs/queries/.../Study_... --new-feed "Brain MRI"
pacs pull /net/pacs/queries/.../Study_... --new-feed "Brain MRI" \
  --pipeline brain-preprocessing -- --segmentation.threshold 0.5
```

Without `--new-feed`, `pacs pull` materializes the selected series under
`/SERVICES/PACS/` and stops. `--new-feed` waits for the complete selection,
resolves every series directory, and creates exactly one named Feed rooted at
`pl-dircopy`. It then prints the Feed ID, root job ID, input series count, and
Feed path.

Feed creation is all-or-nothing. An invalid or empty operand, partial retrieval,
or unresolved CUBE directory prevents Feed creation while leaving any retrieved
files in storage. `--new-feed` cannot be combined with `--nowait`.

`--plugin` and `--pipeline` optionally attach one analysis to the new root and
are mutually exclusive. Arguments after `--` retain the selected executable's
normal grammar. Attachment failure returns nonzero but preserves and reports the
valid Feed and root. See
link:pacsqr.adoc#planned-analysis-attachment[PACS Query / Retrieve / Resolve]
...
Browses and searches the ChRIS peer store for available plugins.

**Syntax:**
```bash
store <subcommand> [options]
```

**Subcommands:**

* `store list` - List all plugins in the store
* `store search <query>` - Search for plugins by name

See link:store.adoc[store command documentation] for full details.

== System Commands

=== id

Prints the current authenticated ChRIS user using Unix-style identity notation.
...
CUBE exposes a numeric user ID but does not define a primary group. ChELL uses
the same POSIX projection as `/etc/passwd`: the primary GID and group name mirror
the user's ID and username. The `groups` field includes that projected primary
group followed by every CUBE group membership, ordered by numeric group ID.
