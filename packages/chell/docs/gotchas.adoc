...
The `!` shell escape executes on the **issuing ChELL surface**, not ChRIS. For
ordinary ChELL that is the local process. For `chell --remote`, it is still the
client machine where ChELL is running--never the CALYPSO daemon machine:
...
# Even through --remote, this prints the client machine's directory
$ ! pwd
----
