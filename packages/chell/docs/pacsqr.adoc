...
|`<queryDesc>`
|Stringified DICOM key-value pairs from the query (e.g. `AccessionNumber:12345678`).
...
[source]
----
query AccessionNumber:12345678 --title "Brain MRI Jan 2024"
...
✓ Query 2679 complete
  ...
  VFS path: /net/pacs/queries/AccessionNumber:12345678_qid:2679_rudolphpienaar
  cd /net/pacs/queries/AccessionNumber:12345678_qid:2679_rudolphpienaar
  pull /net/pacs/queries/AccessionNumber:12345678_qid:2679_rudolphpienaar
----
...
resolved CUBE paths and file counts. Pass `--new-feed "TITLE"` to make that complete retrieved
selection the root input of one new analysis feed.

[source]
----
pull /net/pacs/queries/AccessionNumber:12345678_qid:2679_rudolphpienaar/Study_.../Series_..._AX_T2

 AccessionNumber:12345678|MR-Brain|AX_T2_TSE [DONE]> [████████████████] 68/68

✓ 1/1 series pulled successfully.
  AX_T2_TSE  ->  /SERVICES/PACS/PACSDCM/7654321-.../00018-AX_T2_TSE-90c176c  (68 files)
----

=== Creating a feed from the selection

`--new-feed` is an active operation, not merely feed metadata. Without it, `pull` stops after
materialising the selected series in CUBE storage. With it, `pull` waits for every selected series,
resolves their concrete `/SERVICES/PACS/...` directories, and passes that exact directory set to
`pl-dircopy` as the root of one named feed:

[source,shell]
----
pacs pull /net/pacs/queries/.../Study_... --new-feed "Brain MRI"

✓ 4/4 series pulled successfully.
Feed created: 2326
Root job: pl-dircopy (ID: 176660)
Input: 4 PACS series
Feed path: /home/rudolphpienaar/feeds/feed_2326/pl-dircopy_176660/data/
----

Feed creation is all-or-nothing. An invalid or empty selection operand, a partial retrieval, or an
unresolved CUBE series path leaves any retrieved files in storage but does not create the requested
feed. Because CUBE paths must exist before `pl-dircopy` can consume them, `--new-feed` cannot be
combined with `--nowait`.

[[planned-analysis-attachment]]
=== Planned analysis attachment

`--new-feed` is deliberately the operation that creates the analysis context. A future extension
will optionally attach exactly one plugin or one registered pipeline to the newly created
`pl-dircopy` root:
...
pacs pull <selection...> \
  ...
  --pipeline brain-preprocessing \
  -- \
  --paramFile parameter-overrides.yaml
----

This syntax is a design contract, not an implemented command surface yet. The current parser
rejects `--plugin` and `--pipeline` instead of silently ignoring them. Runtime help therefore
advertises only `--new-feed` until attachment support lands.

The planned rules are:

* `--plugin` and `--pipeline` are mutually exclusive and each requires `--new-feed`. Neither flag
  implicitly creates a feed.
* The selector is resolved exactly as it is for direct plugin execution or `pipeline run`.
* Arguments after `--` are forwarded using the selected plugin or pipeline's existing invocation
  semantics; `pacs pull` does not invent a second parameter grammar. Pipeline invocation uses
  `--<node>.<parameter> <value>` compound parameters or `--paramFile <file>` as specified in
  link:../../../docs/feed-dag-viewer.adoc#pipeline-parameter-binding[Pipeline invocation as
  parameter binding].
* The lifecycle is ordered: retrieve the complete selection, resolve every CUBE directory, create
  the feed and `pl-dircopy` root, then attach the selected analysis to that root.
* Retrieval, resolution, or feed-creation failure prevents attachment. If attachment itself fails,
  the successfully created feed is retained, the command returns nonzero, and output identifies the
  feed/root so the user can continue manually. There is no rollback of a valid feed.
* Successful output reports the feed and root identities already shown above plus the created plugin
...
[source]
----
pull --retry 3 /net/pacs/queries/AccessionNumber:12345678_qid:2679_rudolphpienaar
...
[source]
----
cubepath /net/pacs/queries/AccessionNumber:12345678_qid:2679_rudolphpienaar
...
----
SERVICES/PACS/PACSDCM/7654321-.../00018-AX_T2_TSE-90c176c
----
...
`--new-feed` creation from resolved CUBE directories.
