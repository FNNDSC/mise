...
Physical mode bypasses the PathMapper's hierarchical caching system, using direct link resolution instead. Links are still followed, but without the caching overhead. This provides a simpler, faster path resolution for scenarios where caching isn't beneficial.
...
[source,bash]
----
# User navigates using links or physical paths
cd /home/user/public/data
...
# Note: No PathMapper caching, just direct link resolution
----
...
# Physical: Resolves public -> /PUBLIC but doesn't cache the mapping
----
