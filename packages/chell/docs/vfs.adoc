...
async getData(path?: string): Promise<Result<ListingItem[]>> {
  try {
...
* ✅ Composable: getData() output feeds other operations
...
│  - getData(): fetches and returns data      │
│  - getDataVirtualBin(): /bin plugin listing │
│  - getDataNative(): native ChRIS paths      │
...
[source,typescript]
----
async getData(targetPath?: string, options: {...}): Promise<Result<ListingItem[]>> {
  ...
  if (effectivePath === '/bin') {
    return await this.getDataVirtualBin(options);  // Virtual directory
  } else {
    return await this.getDataNative(effectivePath, options); // Native path
  }
}
----
...
[source,typescript]
----
private async getDataVirtualBin(options): Promise<Result<ListingItem[]>> {
  try {
...
[source,typescript]
----
private async getDataNative(target: string, options): Promise<Result<ListingItem[]>> {
  try {
...
----
async getData(path?: string): Promise<Result<ListingItem[]>>
async getDataVirtualBin(options): Promise<Result<ListingItem[]>>
async getDataNative(path: string, options): Promise<Result<ListingItem[]>>
----
...
async list(path?: string, options): Promise<void> {
  const result = await this.getData(path, options);
...
[source,typescript]
----
const result = await vfs.getData('/home/user');
...
==== `getData(targetPath?: string, options?): Promise<Result<ListingItem[]>>`
...
const result = await vfs.getData('/home/user', { sort: 'date', reverse: true });
if (!result.ok) {
...
==== `getVirtualBinItems(): Promise<ListingItem[]>`

Legacy method for backward compatibility. Returns `/bin` items directly.

**Deprecated:** Use `getData('/bin')` instead for Result<T> pattern.
...
[source,typescript]
----
const plugins = await vfs.getVirtualBinItems();
...
* `getDataVirtualBin(options): Promise<Result<ListingItem[]>>` - Handles `/bin`
* `getDataNative(target, options): Promise<Result<ListingItem[]>>` - Handles native paths
...
Use `getData()` when you need to process data:

[source,typescript]
----
async function countTextFiles(dir: string): Promise<number> {
  const result = await vfs.getData(dir);
...
async function wildcard_expand(pattern: string): Promise<Result<string[]>> {
  ...
  if (!items) {
    ...
    const result = await vfs.getData(cwd);
    if (!result.ok) {
...
async function completer_prefetch(dir: string): Promise<void> {
  ...
  if (result.ok) {
    // Cache populated by getData() - ready for tab completion
    const listCache = listCache_get();
...
[source,typescript]
----
const result = await vfs.getData(path);
if (!result.ok) {
...
describe('VFS error handling', () => {
  it('should return Err for nonexistent path', async () => {
    const result = await vfs.getData('/nonexistent');
...
const items = await vfs.getData('/bin'); // Just data
await vfs.list('/bin'); // Rendered output
----

=== Migration Steps

. **Replace direct method calls** with `getData()` + `list()`
...
=== Why Separate getData() from list()?
...
Caching happens in `getDataVirtualBin()` and `getDataNative()`, not in `list()`:

**Pros:**

* Cache shared across all callers (wildcard, tab completion, list)
* getData() returns cached data for any use case
...
async watch(path: string, callback: (result: Result<ListingItem[]>) => void) {
  ...
  if (changes) {
    ...
    const fresh = await this.getData(path);
    callback(fresh);
  }
}
----
