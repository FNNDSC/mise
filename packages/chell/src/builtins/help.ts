...
const helpText: Record<string, CommandHelp> = {
  ...
  plugin: {
    ...
    description: 'Manage ChRIS plugins (list, run, add)',
    options: [
      '',
      'SUBCOMMANDS:',
      '  list              List all plugins',
      '  run <plugin>      Execute a plugin instance',
    ...
    examples: [
      '# List plugins',
      'plugin list',
      'plugin list --search name:dircopy',
      'plugin list --fields name,version,dock_image',
      '',
      '# Run plugin',
      ...
      'plugin add pl-dircopy --compute host,gpu',
      '',
      '# Add plugin with admin credentials (skip prompt)',
      'plugin add pl-dircopy --adminUser chris --adminPassword chris1234',
      '',
      '# Add plugin from Docker image',
      'plugin add fnndsc/pl-dircopy:2.1.1',
      'plugin add fnndsc/pl-dircopy:2.1.1 --compute host',
      '',
      '# Add plugin from custom peer store',
      'plugin add pl-custom --store https://my-cube.org/api/v1/',
      '',
      '# Add plugin with specific repository',
      'plugin add my-plugin:latest --publicRepo https://github.com/org/my-plugin',
    ],
  },
  plugins: {
