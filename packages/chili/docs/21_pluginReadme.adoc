...
Each plugin should have a repository `README` containing its usage and other
important information. ChILI discovers `README.md` and `README.rst` on the
repository's `main` or `master` branch.

The `plugin` subcommand returns the README for an installed plugin. It uses the
plugin's `public_repo` metadata as the canonical repository and falls back to
`documentation` for older CUBE records. Markdown is rendered for the terminal;
reStructuredText is preserved instead of being passed through a Markdown
parser.
...
A successful README call depends on correct plugin metadata. New registrations
should set `public_repo` to the source repository URL; `documentation` is only a
compatibility fallback. The repository must expose `README.md` or `README.rst`
on `main` or `master`.

In ChELL, a versioned plugin executable exposes the same documentation with
`<plugin>-v<version> --readme`. Add `--raw` when the original README content,
without terminal styling or status text, is needed for a pipe or redirect.
