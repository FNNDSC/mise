...
async function listInodeResources(path: string, options: FSCLIoptions): Promise<void> {
  const chrisInode: ChRISinode|null = await ChRISinode_create(path);
  const params: ListOptions = optionsToParams(options);
  const browserTypes: BrowserType[] = [BrowserType.Files, BrowserType.Links, BrowserType.Dirs];
  const fieldOptions = {
  ...
  for(const browserType of browserTypes) {
    ...
    if(!browser || !browser._resource) {
      console.error(`WARNING: ${browserType} browser or its resource is not available`);
      continue;      
    }
    try {
