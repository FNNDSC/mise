import { feed_doCreate } from '../../../src/commands/feed/create';
...
describe('commands/feed/create', () => {
  ...
  it('should call salsa.feed_create with parsed params and dirs', async () => {
    ...
    const result = await feed_doCreate(options);
  ...
  it('should throw error if dirs are not provided', async () => {
    ...
    await expect(feed_doCreate(options)).rejects.toThrow('Directories for feed creation are required');
  });
  ...
  it('should throw error if parsing params fails', async () => {
    ...
    await expect(feed_doCreate(options)).rejects.toThrow('Error parsing feed parameters: Error: Parsing failed');
  });
