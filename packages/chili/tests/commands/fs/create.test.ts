import { files_doCreate } from '../../../src/commands/fs/create';
...
describe('commands/fs/create', () => {
  ...
  it('should throw error if no fileIdentifier or name is provided', async () => {
    ...
    await expect(files_doCreate(undefined, options)).rejects.toThrow('Filename or path is required.');
  });
  ...
  it('should throw error if both --content and --from-file are used', async () => {
    ...
    await expect(files_doCreate('file.txt', options)).rejects.toThrow('Cannot use both --content and --from-file. Please choose one.');
  });

  it('should call files_create with string content and resolved path', async () => {
    const options: CLIoptions = { content: 'hello' };
    const result = await files_doCreate('file.txt', options);
  ...
  it('should throw an error if files_create returns false', async () => {
    ...
    await expect(files_doCreate('file.txt', options)).rejects.toThrow();
  });

  it('should throw error if local file not found', async () => {
    (fs.existsSync as jest.Mock).mockReturnValue(false);
    const options: CLIoptions = { fromFile: 'nonexistent.txt' };
    await expect(files_doCreate('file.txt', options)).rejects.toThrow('Local file not found at nonexistent.txt');
  });
  ...
  it('should re-throw errors from files_create with an "Error creating file" prefix', async () => {
    ...
    await expect(files_doCreate('file.txt', options)).rejects.toThrow('Error creating file: Salsa error');
  });
  ...
  it('should pass fileIdentifier and options correctly to path_resolve_chrisfs', async () => {
    ...
    await files_doCreate(fileIdentifier, options);
