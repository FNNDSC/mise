...
console.log(`Created retrieve ${retrieve.value.id}`);

// Check status
const status = await pacsRetrieve_statusForQuery(queryId);
if (status.ok) {
  for (const study of status.value.studies) {
...
const retrieves = await pacsRetrieves_list(queryId);
if (retrieves.ok) {
  console.log(`Found ${retrieves.value.length} retrieves`);
}
```
