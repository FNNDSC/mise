...
- `PACSQueryCreateData`: `{ title: string; query: string; description?: string }` where `query` is a JSON string of DICOM tag filters (e.g., `{"PatientID":"7654321"}`).
...
const createdQuery: Result<PACSQueryRecord> = await pacsQueries_create("PACSDCM", {
  title: "PID 7654321",
  query: JSON.stringify({ PatientID: "7654321" }),
});
